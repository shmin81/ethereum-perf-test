...
const bytecode = fs.readFileSync(
	//path.join(__dirname, "../contracts/DocManager.bin")
	path.join(__dirname, "../contracts/DocManagerMulti.bin")
);
...
async function run() {
	...
	try {
        ...
        if (txResults.status == true) {
			...
			utils.deployNewDocMgmtContract(txResults.contractAddress, txResults.transactionHash)
        }
	}
	catch (err) {
		LOG(err); 
